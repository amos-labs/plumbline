...
test("attempt history: first rerun archives the prior comment under a details block", () => {
  const first = renderComment(gate("rework", ["diff_sha256 mismatch: receipt=aa actual=bb"]));
...
test("attempt history: grows newest-first across reruns", () => {
  const a1 = renderComment(gate("rework", ["error one"]));
  const a2 = renderComment(gate("rework", ["error two"]));
...
test(`attempt history: capped at ${HISTORY_CAP}`, () => {
  let body = renderComment(gate("rework", ["error 0"]));
  for (let i = 1; i <= HISTORY_CAP + 3; i++) {
    body = appendAttemptHistory(renderComment(gate("rework", [`error ${i}`])), body, NOW);
  }
...
test("attempt history: truncation re-balances details tags", () => {
  const huge = renderComment(gate("rework", ["x".repeat(6000)]));

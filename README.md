...
npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3" init
...
GITHUB_TOKEN=<repo-admin token> npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3" setup-protection --repo owner/name
...
```bash
npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3" propose "Rotate auth session tokens" --body "Tokens never expire today."
...
```bash
npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3" receipt --write   # one idempotent step: scaffold .plumbline/receipts/<branch>.json
...
npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3" check             # local pre-flight — the SHAPE floor + diff_sha256 only (fast, offline, free).
                                                 # NOT the full verdict: the LLM semantic review runs in CI.
npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3" check --review    # full parity — also runs the semantic review locally for the real verdict
...
```bash
npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3" archive rotate-auth-session-tokens
...
[Quick start](#quick-start-agent-installable) above (`npx -y "git+https://github.com/amos-labs/plumbline#v0.7.3"`). Use the `git+https…#tag` spec form: `github:owner/repo@tag` is silently mis-parsed by npm ≤ 10 (the Node 20 default) into `null/<tag>` and exits 128 with no output at all.
